...
class PatchDataset(torch.utils.data.Dataset):
    ...
    '''
    def __init__(self, img: np.ndarray, s:int, s1:int, normalizer=None, check_empty_patches=False):
        self.img = img
        self.has_alpha = img.shape[0] == 4 #if img has 4 channels, means it has alpha mask
        self.s = s
        self.s1 = s1
        assert s > s1
    ...
    def __getitem__(self, idx):
        x, i, j = self.load(idx)
        x = x.transpose([1,2,0]) # transpose to later use torchvision ToTensor to convert back to [c,s,s] and scale to range [0., 1.] 
        ...
        for sub_im_normalized, (i,j) in dataloader:
            ...
            for k in range(len(out)):
                try:
                    ...
                    big_out[:, i[k]+di1:i[k]+di2, j[k]+dj1:j[k]+dj2] = copy.deepcopy(out[k][:, di1:di2, dj1:dj2])
...
class VegProcessor(Processor):
    ...
    def __init__(self, ref_stats_filepath='models/traindata_hist_quantile_mean_std(veg_only).pkl'):
    ...
    def preprocess(self, img: np.ndarray) -> np.ndarray:
        ...
        if img.shape[0] == 3:
            ...
            log.info(f'Fourth channel detected, which has min = {img[-1].min()}, max={img[-1].max()}. It will be treated as alpha channel')
    ...
    def patchify(self, img: np.ndarray, s:int, s1:int) -> None:
        ...
        '''        
        self.dataset = PatchDataset(img, s, s1, IMAGENET_NORMALIZER, check_empty_patches=True)
        self.dataloader = torch.utils.data.DataLoader(self.dataset, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, prefetch_factor=PREFETCH_FACTOR, persistent_workers=False)
        return None
    ...
    def infer_veg_segmentation(self, img: np.ndarray, standardize=False) -> np.ndarray:
        ...
        allveg = inference(
            self.allveg_model, 
            self.dataloader, 
            n_classes=1, device='cpu', 
            segmentation_class='allveg10m')

        return allveg #[1, h, w], uint8, 0=nodata, 1=non-veg, 2=low-veg, 3=high-veg
    
    def infer_height_estimation(self, img: np.ndarray[np.uint8], standardize=False) -> np.ndarray:
        # img shape: [3, h, w] uint8
        # steps: 1. Standardize img (optional) - 2. patchify, create dataloader - 3. infer, reassemble to large array
    ...
    def process(self, img: np.ndarray | Dataset) -> Dataset:
        ...
        self.mask = np.bitwise_and(self.mask, ~nonvegmask[0]) # remember self.mask.shape=[h,w]
        nonvegmask = np.repeat(nonvegmask, 3, axis=0)
        img[nonvegmask] = 0
        # Step 3: infer standardized data with height_model
        log.info('step 3')
        height = self.infer_height_estimation(img, standardize=True) #height [1, h, w] float16
...
def get_mask(im, channel_axis=0, nodata=0):
    ...
    '''
    mask = np.repeat(np.all(im == nodata, axis=channel_axis, keepdims=True), repeats=im.shape[channel_axis], axis=channel_axis)
    return mask
